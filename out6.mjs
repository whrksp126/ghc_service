...
var require_react_production_min = __commonJS({
  "frontend/node_modules/react/cjs/react.production.min.js"(exports) {
    "use strict";
    ...
    E.prototype.setState = function(a, b3) {
    ...
    function M(a, b3, e) {
      ...
      if (null != b3) for (d in void 0 !== b3.ref && (h = b3.ref), void 0 !== b3.key && (k = "" + b3.key), b3) J.call(b3, d) && !L.hasOwnProperty(d) && (c[d] = b3[d]);
    ...
    function R(a, b3, e, d, c) {
      ...
      if (h) return h = a, c = c(h), a = "" === d ? "." + Q(h, 0) : d, I(c) ? (e = "", null != a && (e = a.replace(P, "$&/") + "/"), R(c, b3, e, "", function(a2) {
        return a2;
      })) : null != c && (O(c) && (c = N(c, e + (!c.key || h && h.key === c.key ? "" : ("" + c.key).replace(P, "$&/") + "/") + a)), b3.push(c)), 1;
      h = 0;
      d = "" === d ? "." : d + ":";
      ...
      else if (f = A(a), "function" === typeof f) for (a = f.call(a), g = 0; !(k = a.next()).done; ) k = k.value, f = d + Q(k, g++), h += R(k, b3, e, f, c);
      else if ("object" === k) throw b3 = String(a), Error("Objects are not valid as a React child (found: " + ("[object Object]" === b3 ? "object with keys {" + Object.keys(a).join(", ") + "}" : b3) + "). If you meant to render a collection of children, use an array instead.");
      return h;
    }
    function S2(a, b3, e) {
      if (null == a) return a;
    ...
    exports.Children = { map: S2, forEach: function(a, b3, e) {
      S2(a, function() {
    ...
    exports.act = X;
    exports.cloneElement = function(a, b3, e) {
    ...
    exports.memo = function(a, b3) {
      return { $$typeof: x, type: a, compare: void 0 === b3 ? null : b3 };
    };
    ...
    exports.useCallback = function(a, b3) {
      return U.current.useCallback(a, b3);
    };
    ...
    exports.useEffect = function(a, b3) {
      return U.current.useEffect(a, b3);
    };
    exports.useId = function() {
      return U.current.useId();
    };
    exports.useImperativeHandle = function(a, b3, e) {
      return U.current.useImperativeHandle(a, b3, e);
    };
    exports.useInsertionEffect = function(a, b3) {
      return U.current.useInsertionEffect(a, b3);
    };
    exports.useLayoutEffect = function(a, b3) {
      return U.current.useLayoutEffect(a, b3);
    };
    exports.useMemo = function(a, b3) {
      return U.current.useMemo(a, b3);
    };
    exports.useReducer = function(a, b3, e) {
      return U.current.useReducer(a, b3, e);
    };
    ...
    exports.useSyncExternalStore = function(a, b3, e) {
      return U.current.useSyncExternalStore(a, b3, e);
    };
...
var useAuthStore = create()(
  persist(
    (set, get) => ({
      ...
      deviceId: null,
      deviceLabel: null,
      setAuth: (token, userId, nickname, email) => set({ token, userId, nickname, email: email ?? null }),
      setDevice: (deviceId, deviceLabel) => set({ deviceId, deviceLabel }),
      logout: () => set({ token: null, userId: null, nickname: null, email: null, deviceId: null, deviceLabel: null }),
...
var nextFxId = 1;
function withBoard(s, boardId, fn) {
  const board = s.boards[boardId];
  if (!board) return s;
  return { ...s, boards: { ...s.boards, [boardId]: fn(board) } };
}
function pruneEffects(boards) {
  const now = Date.now();
  const out = {};
  for (const [id, b3] of Object.entries(boards)) {
    out[id] = b3.effects.length > 0 ? { ...b3, effects: b3.effects.filter((e) => e.until > now) } : b3;
  }
  return out;
}
function myUserId() {
...
var useGameStore = create()((set, get) => ({
  ...
  applyMatched: (e) => {
    ...
    if (clashed) {
      set((prev) => ({
        pendingPicks: prev.pendingPicks.map(
          (p) => p.boardId === e.boardId && (p.a === e.a || p.a === e.b || p.b === e.a || p.b === e.b) ? { ...p, superseded: true, supersededColor: winnerColor } : p
        )
      }));
    }
    set((prev) => {
      if (!prev.snapshot) return prev;
      let next = withBoard(prev.snapshot, e.boardId, (b3) => {
        const cells = b3.cells.slice();
        cells[e.a] = 0;
        cells[e.b] = 0;
        return { ...b3, cells, remaining: e.remaining };
      });
      next = {
        ...next,
        seq: e.seq,
  ...
  applyShuffled: (e) => {
    ...
    set((prev) => {
      if (!prev.snapshot) return prev;
      const next = withBoard(prev.snapshot, e.boardId, (b3) => ({ ...b3, cells: e.cells.slice() }));
  ...
  applyAttack: (e) => {
    ...
    if (e.seq <= s.seq) return true;
    if (e.seq > s.seq + 1) return false;
    const already = get().snapshot?.boards[e.boardId]?.effects.some((x) => x.type === e.type && x.until === e.until);
    set((prev) => {
      if (!prev.snapshot) return prev;
      const now = Date.now();
      const next = withBoard(prev.snapshot, e.boardId, (b3) => {
        const others = b3.effects.filter((x) => x.type !== e.type && x.until > now);
        const sameType = b3.effects.filter((x) => x.type === e.type && x.until > now);
        const merged = sameType.reduce(
          (best, x) => x.until > best.until ? x : best,
          { type: e.type, until: e.until, hidden: e.hidden }
        );
        return { ...b3, effects: [...others, merged] };
      });
  ...
  predictPick: (boardId, a, b3, path, color) => {
    const s = get().snapshot;
    const board = s?.boards[boardId];
    if (!s || !board) return;
    ...
    set({
      ...
      selectedIdx: null,
      hintPair: null,
      pendingPicks: [...get().pendingPicks, { a, b: b3, sym, boardId }]
    });
    ...
    get().pushFx({
      ...
      symbol: sym,
      color,
      combo: (get().me()?.combo ?? 0) + 1,
      fromUserId: myUserId() ?? void 0
    });
  },
  dropPending: (pick2, flashColor) => {
    set({
      pendingPicks: get().pendingPicks.filter((p) => !(p.a === pick2.a && p.b === pick2.b && p.boardId === pick2.boardId))
    });
    get().pushFx({
      type: "flash",
      boardId: pick2.boardId,
      cells: [pick2.a, pick2.b],
      color: flashColor ?? pick2.supersededColor ?? "#FFFFFF"
    });
  },
  rollbackPick: (pick2) => {
    const s = get().snapshot;
    if (!s || pick2.superseded) {
      get().dropPending(pick2);
      return;
    }
    set({
      snapshot: withBoard(s, pick2.boardId, (bd) => {
        const cells = bd.cells.slice();
        if (cells[pick2.a] === 0) cells[pick2.a] = pick2.sym;
        if (cells[pick2.b] === 0) cells[pick2.b] = pick2.sym;
        let remaining = 0;
        for (const v of cells) if (v !== 0) remaining++;
        return { ...bd, cells, remaining };
      }),
      pendingPicks: get().pendingPicks.filter((p) => !(p.a === pick2.a && p.b === pick2.b))
    });
    get().pushFx({ type: "invalid", boardId: pick2.boardId, cells: [pick2.a, pick2.b] });
  },
  takePending: (a, b3) => {
    const hit = get().pendingPicks.find(
      (p) => p.a === a && p.b === b3 || p.a === b3 && p.b === a
    );
    if (!hit) return null;
  ...
  myBoard: () => {
    ...
    return meState ? s.boards[meState.boardId] : void 0;
  },
  reset: () => set({
    snapshot: null,
    selectedIdx: null,
...
if (false) {
  window.__ghcGame = { store: useGameStore, engine };
}

// entry6.ts
useAuthStore.setState({ userId: "u1", nickname: "me" });
var snap = {
  gameId: "shisen",
  phase: "playing",
  hostUserId: "u1",
  mode: "coop",
  options: { boardSize: "s", items: false, timeLimitSec: 0 },
  seed: 1,
  startAt: Date.now(),
  endedAt: null,
  players: [
...
var pend = useGameStore.getState().pendingPicks[0];
console.log("pending superseded?", pend?.superseded, "color", pend?.supersededColor);
var pick = useGameStore.getState().takePending(0, 1);
useGameStore.getState().rollbackPick(pick);
var b = useGameStore.getState().snapshot.boards.shared;
...
var p2 = useGameStore.getState().takePending(4, 5);
useGameStore.getState().rollbackPick(p2);
var b2 = useGameStore.getState().snapshot.boards.shared;
