...
start_abr() {
  ffmpeg -nostdin -loglevel warning -rtsp_transport tcp -i "$input" \
    -map 0:v -map '0:a?' -c copy -f flv "rtmp://127.0.0.1:1935/abr1080/${key}" \
    -map 0:v -map '0:a?' \
    ...
    -force_key_frames source -c:a copy -f flv "rtmp://127.0.0.1:1935/abr720/${key}" &
  abr_pid=$!
}
...
# Restart only the failed branch. The single ABR branch creates both HLS qualities from ONE reader,
# keeping timestamps/keyframes aligned while remaining isolated from LiveKit backpressure.
while :; do
